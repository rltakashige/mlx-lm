...
# ============================================================
# mlx-lm chat_template_type entry point
# ============================================================
...
def apply_chat_template(
    ...
    **kwargs,
    ...
    """mlx-lm entry point. ``tokenizer_config["chat_template_type"] =
    "deepseek_v4"`` routes ``tokenizer.apply_chat_template`` here.

    Semantics:
    - ``add_generation_prompt=True`` (the default in most generation flows)
      leaves the rendered string ending with ``<|Assistant|><think>`` (or
      ``</think>`` in chat mode) so the model starts the assistant turn.
    - ``add_generation_prompt=False`` strips the trailing assistant-turn-start
      tokens, matching how other mlx-lm chat templates behave for training.
    - ``continue_final_message=True`` drops the trailing ``<|end_of_sentence|>``
      so the model resumes an in-flight assistant turn.
    """
    ...
    # The reference merge is done inside encode_messages; it expects raw
    # OpenAI-format messages (tools live on a separate kwarg).
    if tools is not None:
        messages = [dict(m) for m in messages]
        # Attach tools to the first system or user message so render_message
        # picks them up via msg.get("tools"), mirroring the encoding helper.
        if messages and messages[0].get("role") == "system":
            messages[0].setdefault("tools", tools)
        elif messages:
    ...
    if not add_generation_prompt and messages and messages[-1].get("role") == "user":
        # Strip the assistant-turn-start that render_message appended.
        start = ASSISTANT_SP_TOKEN + (
            thinking_start_token if thinking_mode == "thinking" else thinking_end_token
        )
        out = out.removesuffix(start)
